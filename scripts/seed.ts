// CryoVault Database Seed Script
...
async function main() {
  ...
  // Create 20 freezer slots (5 rows x 4 columns)
  console.log("Creating 20 freezer slots...")
  const slots = []

  for (let row = 1; row <= 5; row++) {
    for (let col = 1; col <= 4; col++) {
      const name = `F${row}-${String.fromCharCode(64 + col)}` // F1-A, F1-B, etc.
      ...
      // Base temperature around -70°C with variance
      const baseTemp = -70 + (Math.random() - 0.5) * 4

      // Determine temp status based on deviation
  ...
  // Create some sample bookings for occupied slots
  ...
  // Create some pending reservations for reserved slots
